...
class CarSupplier(SupplierBase):
    def __init__(self, args):
        super(CarSupplier, self).__init__(args)
        all_classes = sorted(os.listdir(os.path.join(self.root_path, 'web_data')))

        if args.novel_split < 0:
            print('default split')
            for i, c in enumerate(all_classes):
                if i % 4 == 0:
                    self.novel_classes.append(c)
                else:
            ...
            self.base_classes = all_c[args.novel_split:]
            self.novel_classes = all_c[:args.novel_split]

    def clean_dataset_base(self, mode):
        assert mode in ["train", "test"]
        if mode == "train":
            return CarTrainSet(self.root_path, self.base_classes, self.train_transforms)
        else:
            return CarTestSet(self.root_path, self.base_classes, self.test_transforms)

    def clean_dataset_novel(self, mode):
        assert mode in ["train", "test"]
        if mode == "train":
            return CarTrainSet(self.root_path, self.novel_classes, self.train_transforms)
        else:
            return CarTestSet(self.root_path, self.novel_classes, self.test_transforms)
...
class CarTrainSet(BaseDataset):
    def __init__(self, root_path, classes, transform=None):
        super(CarTrainSet, self).__init__(root_path, classes, transform)
        self.load(root_path, classes)

    def load(self, root_path, classes):
        ...
        for path in all_train_images:
            name = id_to_name[int(path.split('/')[1])]
            if name in classes:
                self.image_list.append((os.path.join(root_path, 'CampCar/image', path), self.cls2int[name]))


class CarTestSet(BaseDataset):
    def __init__(self, root_path, classes, transform=None):
        super(CarTestSet, self).__init__(root_path, classes, transform)
        self.load(root_path, classes)
    ...
    def load(self, root_path, classes):
        ...
        for idx, name in id_to_name.items():
            if name != 'unknown' and name in classes:
                name_to_id[name] = idx
...
class WebSet(BaseDataset):
    ...
    def load(self, classes, max_noisy_images_per):
        self.image_list = []
        for cls in classes:
            dir_path = os.path.join(self.root_path, 'web_data', cls)
            ...
            cls_list = [(os.path.join(dir_path, image_name), self.cls2int[cls]) for image_name in image_names]

            self.image_list += cls_list[:max_noisy_images_per]
